/*
 * lockwise_mutual_2c_pool.h
 *
 * A simple thread pool using a mutual task queue within each worker thread,
...
#ifndef LOCKWISE_MUTUAL_2C_POOL_H
#define LOCKWISE_MUTUAL_2C_POOL_H
...
class Thread_Pool {
  ...
  private:
    ...
    thread* _workers_;
    Lockwise_Deque<Task_Wrapper>* _workerqueues_;
        ...
        while (!_done_.load(memory_order_acquire)) {
            if (_workerqueues_[index].pop(task))
                ...
                for (unsigned i = 0; i < _workersize_; ++i)
                    if (_workerqueues_[(index + i + 1) % _workersize_].pull(task)) {
                        task();
                        break;
                    }
  ...
  public:
    Thread_Pool() : _suspend_(false), _done_(false) {
        try {
            ...
            _workerqueues_ = new Lockwise_Deque<Task_Wrapper>[_workersize_]();
            for (unsigned i = 0; i < _workersize_; ++i)
